<#
	HTTP BASE functions
		...
		Estas são as funcoes HTTP base que usaremos no PowerZabbix.  
		...
		As funcoes HTTP são disponibilizadas em 3 cmdlets:
			...
			- Receive-HttpRequest   
				Recebe o request criado por StartHttpRequest 
				
			- Invoke-Http 
				É um atalaho para StartHttpRequest + Receive-HttpRequest.  
...
Function StartHttpRequest {
	...
	if($data -and 'POST','PATCH','PUT' -Contains $method){
		...
		if($contentType -eq "multipart/form-data"){
			...
			#Itera...
			$BodyLines = @()
			#Vamos usar um boundary com esse formato: __PowershellInvokeHttp__GUID.
			$boundary = "__PowershellInvokeHttp_" + [System.Guid]::NewGuid().ToString();
...
function GetHttpResponse {
	...
	try {
		...
		CloseHttpRequest $HttpRequest -Force:$ForceEnd
	}
...
function CloseHttpRequest {
	<#
		.SYNOPSIS 
			Close Http opended with StartHttpRequest
	#>
	[CmdLetBinding()]
	param(
		[Parameter(ValueFromPipeline=$true)]
...
function InvokeHttp {
	...
	try {
		...
		while($true){
			...
			$HttpResult = GetHttpResponse @ResponseParams
		...
		CloseHttpRequest -Force $HttpRequest
	}
