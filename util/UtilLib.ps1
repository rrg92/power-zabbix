...
function GetProjectVars {
	...
	$ProjectVars = get-item "project.util.ps1" -EA SilentlyContinue

	if(!$ProjectVars){
		throw "CurrentDir must be project root";
	}
	
	#run 
	$Vars = & $ProjectVars.FullName;
	$ModuleDir = $Vars.ModuleDir;
	
	if(-not(Test-Path (Resolve-Path "./$ModuleDir"))){
		throw "MODULEDIR_NOTFOUND: $ModuleDir";
	}
...
function ParseCommandHelp {
	...
	foreach($Syntax in $SyntaxItemsBackup){
		...
		if(!$MySet){
			...
			$msg = "POWERZABBIX_PARSECOMMANDHELP: ParameterSet not found for Syntax. Command = $($Cmd.Name). Param: $($SortedParams)";
